...
def main():
    ...
    try:
        ...
        # AFTER — also stops serve_forever() and exits the process
        def _shutdown_service():
            _service_ready.wait(timeout=60)
            svc = _service_container.get("service")
            if svc is not None:
                svc.shutdown()
            # Schedule server.shutdown() in a background thread — calling it
            # directly from a registered callable can deadlock serve_forever().
            import threading as _t
            def _stop():
                import time as _time
                _time.sleep(0.3)   # allow RPC response to return first
                try:
                    server.shutdown()
                except Exception:
                    pass
